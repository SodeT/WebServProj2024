...
def update_booster(booster_id, name, multiplier, price)
  db = open_db
  db.execute('UPDATE boosters SET name = ?, multiplier = ?, price = ? WHERE id = ?', name, multiplier, price, booster_id)
end
...
def get_event_price(user_id, event_id)
  db = open_db
  db.execute('SELECT users.tokens, events.fee FROM users INNER JOIN events ON events.id = ? WHERE users.id = ?', event_id, user_id).first
end
...
def update_event(event_id, name, reward, condition, fee)
  db = open_db
  db.execute('UPDATE events SET name = ?, reward = ?, condition = ?, fee = ? WHERE id = ?', name, reward, condition, fee, event_id)
end
